...
fn get_cursor_version_macos() -> Result<String, String> {
    ...
    let package_path = PathBuf::from("/Applications/Cursor.app/Contents/Resources/app/package.json");
...
fn get_cursor_version_windows() -> Result<String, String> {
                ...
                if !path_str.is_empty() {
                    ...
                    if exe_path.exists() {
                        ...
                        if let Some(cursor_dir) = exe_path.parent() {
                            let package_path = cursor_dir.join("resources").join("app").join("package.json");
    ...
    let mut cursor_paths = vec![
        PathBuf::from(format!("{}\\Programs\\Cursor\\resources\\app\\package.json", localappdata)),
        PathBuf::from(format!("{}\\Programs\\cursor\\resources\\app\\package.json", localappdata)),
        PathBuf::from(format!("{}\\Cursor\\resources\\app\\package.json", localappdata)),
    ...
    for drive in &["C", "D", "E", "F", "G"] {
        cursor_paths.push(PathBuf::from(format!("{}:\\cursor\\resources\\app\\package.json", drive)));
        cursor_paths.push(PathBuf::from(format!("{}:\\Cursor\\resources\\app\\package.json", drive)));
        cursor_paths.push(PathBuf::from(format!("{}:\\Program Files\\Cursor\\resources\\app\\package.json", drive)));
        cursor_paths.push(PathBuf::from(format!("{}:\\Program Files (x86)\\Cursor\\resources\\app\\package.json", drive)));
        cursor_paths.push(PathBuf::from(format!("{}:\\Software\\Cursor\\resources\\app\\package.json", drive)));
        cursor_paths.push(PathBuf::from(format!("{}:\\Apps\\Cursor\\resources\\app\\package.json", drive)));
        cursor_paths.push(PathBuf::from(format!("{}:\\Programs\\Cursor\\resources\\app\\package.json", drive)));
    }
...
fn get_cursor_version_linux() -> Result<String, String> {
    ...
    let cursor_paths = vec![
        PathBuf::from(format!("{}/.local/share/cursor/resources/app/package.json", home)),
